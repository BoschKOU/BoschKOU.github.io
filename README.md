...
<body>
    ...
    <div class="card">
        ...
        <div class="flex-row">
            <div class="input-group">
                <label>现有总股本（万股）</label>
                <input type="number" id="totalShares" value="10000" step="100">
            </div>
            <div class="input-group">
                <label>创始人投前持股（%）</label>
                <input type="number" id="founderPre" value="30.20" step="0.1">
            </div>
        </div>
        <div class="input-group">
    ...
    <div class="card">
        ...
        <div class="note" style="margin-bottom:10px;">
            ...
            就像在一个网格上挨个检查，找出所有同时满足“综合价4-4.5”和“创始人持股底线”的方案。<br>
            ...
            <table id="scanTable" style="display:none;">
                <thead>
                    <tr>
                        ...
                        <th>综合成本</th><th>投资人股比</th>
                        <th>公司入账(万)</th><th>A套现(万)</th>
                        <th>创始人投后</th>
                    </tr>
                </thead>
                <tbody></tbody>
    ...
    <div class="card">
        <h3>📈 可行域散点图（可视化）</h3>
        <div class="note" style="margin-bottom:10px;">
            输入你想观察的总融资额 T，图表会显示不同老股估值和出让比例下的综合成本。<br>
            绿色点 = 同时满足综合价4.0~4.5 且 创始人投后≥底线。
        </div>
        <div class="flex-row">
            <div class="input-group">
                <label>观察的 T 值（万元）</label>
                <input type="number" id="chartT" value="5000" step="100" onchange="drawChart()">
            </div>
            <div class="input-group">
                <label>创始人最低投后（%）</label>
        ...
        <div class="note">红色虚线：综合成本4.0和4.5。绿点：通过所有约束。</div>
    </div>
    ...
    <script>
        ...
        function judge(blended) {
            if (blended >= 4.0 && blended <= 4.5) return "✅ 综合价可行";
            if (blended < 4.0) return "⚠️ 综合价太低";
            return "⚠️ 综合价太高";
        }
        ...
        function calculate() {
            ...
            const status = judge(res.blended);
        ...
        function scanAll() {
            ...
            for (let T of Tarr) {
                for (let Vold = 25000; Vold <= 45000; Vold += Vstep) {
                    for (let a = aMin; a <= aMax; a += aStep) {
                        const res = calcOne(T, pre, Vold, a, shares, founderPre);
                        if (res.blended < 4.0 || res.blended > 4.5) continue;
            ...
            tbody.innerHTML = '';
            if (results.length === 0) {
                tbody.innerHTML = '<tr><td colspan="8">没有找到满足所有条件的方案，请放宽约束（如降低创始人最低持股、扩大出让比例范围等）。</td></tr>';
            } else {
            ...
            document.getElementById('scanInfo').innerText = `✅ 找到 ${results.length} 个可行方案，按公司进账从多到少排列。`;
        }
        ...
        function drawChart() {
            ...
            if (isNaN(T) || isNaN(pre) || isNaN(founderPre) || isNaN(founderMin) || isNaN(aMin) || isNaN(aMax)) {
                alert("请填写散点图所有参数");
                return;
            }
            ...
            for (let a of aList) {
                ...
                for (let Vold of VoldList) {
                    const res = calcOne(T, pre, Vold, a, shares, founderPre);
                    const feasible = (res.blended >= 4.0 && res.blended <= 4.5 && res.founderPostPct >= founderMin);
                    if (feasible) {
                        feasiblePoints.push({x: Vold, y: res.blended});
                    } else {
                        unfeasiblePoints.push({x: Vold, y: res.blended});
                    }
                }
            ...
            chartInstance = new Chart(ctx, {
                ...
                options: {
                    plugins: {
                        tooltip: {
                            callbacks: {
                                label: (ctx) => `老股估值:${ctx.raw.x}万, 综合成本:${ctx.raw.y.toFixed(2)}`
                            }
                        },
                    ...
                    scales: {
                        ...
                        y: {
                            title: { display: true, text: '综合每股成本（元）' },
                            min: 2.5, max: 6.5
                        }
                    }
                },
